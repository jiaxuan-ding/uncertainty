# -*- coding: utf-8 -*-
"""
Created on Sun Nov  3 11:04:34 2024
...
#demographic
#Lab
data['expb5yr']=0;data['AT2H']=0;data['wrka6day']=0;data['drivingdisB300']=0
data.loc[(data['experience_months']<=54),'expb5yr']=1 
data.loc[(data['Dis_p_day']<=325),'drivingdisB300']=1
data.loc[(data['AT2']<=2),'AT2H']=1
data.loc[(data['Freq_p_week']<=5),'wrka6day']=1
#online
datao['expb5yr']=0;datao['AT2H']=0;datao['wrka6day']=0;datao['drivingdisB300']=0
datao.loc[(datao['experience_months']<=54),'expb5yr']=1 
datao.loc[(datao['Dis_p_day']<=325),'drivingdisB300']=1
datao.loc[(datao['AT2']<=2),'AT2H']=1
datao.loc[(datao['Freq_p_week']<=5),'wrka6day']=1
#combining together
select_demo=['expb5yr','drivingdisB300','AT2H','wrka6day']
demo1=(data.loc[(data['alt']==1),select_demo].to_numpy()[np.arange(0,Nid*Nscenario,Nscenario)]).reshape(Nid,4)
demo2=(datao.loc[(datao['alt']==1),select_demo].to_numpy()[np.arange(0,Nido*Nscenarioo,Nscenarioo)]).reshape(Nido,4)
demo=np.vstack((demo1,demo2))

######eye-tracking data##############
#lab data: aggreated number of fixation on each attribute per person and choice scenario
trans_count=data.loc[((data['alt']==1)),['N_RC','N_OC1','N_OC2','N_DR']].to_numpy()
#lab data: aggreated number of fixation on each attribute per choice scenario
choice_set=data.loc[(data['alt']==1),'Scenario'].to_numpy();dic={}
for i in range(0,10):
    dic[choice_set_index[i]]=np.sum(trans_count[np.where((choice_set==choice_set_index[i]) | (choice_set==choice_set_index[i+10])),:],axis=1)/np.sum(trans_count[np.where((choice_set==choice_set_index[i]) | (choice_set==choice_set_index[i+10])),:])
    dic[choice_set_index[i+10]]=np.sum(trans_count[np.where((choice_set==choice_set_index[i]) | (choice_set==choice_set_index[i+10])),:],axis=1)/np.sum(trans_count[np.where((choice_set==choice_set_index[i]) | (choice_set==choice_set_index[i+10])),:])

#attention weight for lab and online data
w_p_t=np.zeros((Nid*Nscenario+Nido*Nscenarioo,Nattri))
n_indx=0
#lab data
for i in data['ID'].unique():
    for j in data.loc[((data['ID']==i)),'Scenario'].unique():
        w_p_t[n_indx,:]=dic[j][0]
        n_indx+=1
#online data        
for i in datao['ID'].unique():
    for j in datao.loc[((datao['ID']==i)),'Scenario'].unique():
        w_p_t[n_indx,:]=dic[j][0]
        n_indx+=1
#multiply attention weight with the states probability             
w_p_t[0:Nid*Nscenario,1]=w_p_t[0:Nid*Nscenario,1]*(data.loc[(data['alt']==3),['P1']].to_numpy()).flatten();w_p_t[0:Nid*Nscenario,2]=w_p_t[0:Nid*Nscenario,2]*(data.loc[(data['alt']==3),['P2']].to_numpy()).flatten()
w_p_t[Nid*Nscenario:Nid*Nscenario+Nido*Nscenarioo,1]=w_p_t[Nid*Nscenario:Nid*Nscenario+Nido*Nscenarioo,1]*(datao.loc[(datao['alt']==3),['P1']].to_numpy()).flatten();w_p_t[Nid*Nscenario:Nid*Nscenario+Nido*Nscenarioo,2]=w_p_t[Nid*Nscenario:Nid*Nscenario+Nido*Nscenarioo,2]*(datao.loc[(datao['alt']==3),['P2']].to_numpy()).flatten()
temp=np.sum(w_p_t[:,1:3],axis=1);temp1=1-w_p_t[:,0]-w_p_t[:,3]
w_p_t[:,1]=temp1*w_p_t[:,1]/temp ;w_p_t[:,2]=temp1*w_p_t[:,2]/temp
w_p=w_p_t.reshape((Nido+100,Nscenarioo,Nattri,1))
...
#feedback matrix: S
    ...
    # calculate Expectation
        ...
        Sprob_E=Sprob_E.flatten()
        Sprob_EN=np.hstack((np.prod(Sprob_E[0:200].reshape(40,5),axis=1),np.prod(Sprob_E[200:].reshape(152,2),axis=1)))  
        return Sprob_EN

    
def call_lent(param, C, data_att, w_p,Nid, Nscenario,Nalter,Nattri,L,demo):
    pram_mdft_c1=param[0:7];pram_mdft_c2=np.hstack((param[0:2],param[[7,8,4]],param[[9,10]]));parm_laten=param[11:11+demo.shape[1]]#;parm_laten_asc=param[11+demo.shape[1]]
    p_mdft_c1=cal_LL(pram_mdft_c1, C,data_att, w_p,Nid, Nscenario,Nalter,Nattri,L)
    p_mdft_c2=cal_LL(pram_mdft_c2, C, data_att, w_p,Nid, Nscenario,Nalter,Nattri,L)
    V2=np.sum(parm_laten*demo,axis=1)
    P1=(1/(1+np.exp(V2)))
    P2=(np.exp(V2))/(1+np.exp(V2))
    LLH=-np.sum(np.log((P1*p_mdft_c1+P2*p_mdft_c2)))
    return LLH

def trans(params,cov):
    sample_p=np.random.multivariate_normal(params, cov, 1000000)
    phi1 = 1/(1+np.exp(-sample_p[:,0]));phi2=1/(1+np.exp(-sample_p[:,1]));t_step=1+np.exp(sample_p[:,5]);t_step1=1+np.exp(sample_p[:,9])
    return np.std(phi1),np.std(phi2),np.std(t_step),np.std(t_step1)


########################################estimation########################## 
#Calculate likelihood of selection at each time step based on sims
Nfeval = 1
bounds=((-np.Inf,np.Inf),(-np.Inf,np.Inf),(0.1,20),(0.1,20),(0.1,20),(-6,np.Inf),(-np.Inf,np.Inf),(0.1,20),(0.1,20),(-6,np.Inf),(-np.Inf,np.Inf),(-np.Inf,np.Inf),(-np.Inf,np.Inf),(-np.Inf,np.Inf),(-np.Inf,np.Inf))

def callbackF(Xi):
    global Nfeval
    print ('{0:4d}   {1: 3.6f}   {2: 3.6f}   {3: 3.6f}   {4: 3.6f}   {5: 3.6f}   {6: 3.6f}    {7: 3.6f}     {8: 3.6f}    {9:3.6f}   {10: 3.6f}   {11: 3.6f}    {12: 3.6f}   {13: 3.6f}   {14: 3.6f}   {15: 3.6f}   {16: 3.6f}'.format(Nfeval, Xi[0], Xi[1], Xi[2], Xi[3],Xi[4], Xi[5], Xi[6],Xi[7], Xi[8], Xi[9],Xi[10], Xi[11], Xi[12], Xi[13],Xi[14], call_lent(Xi,C, data_att, w_p,Nido+100, Nscenarioo,Nalter,Nattri,L,demo)))
    Nfeval += 1
 

initial=np.array([0.648964323,0.857195942,2.752045272,2.824619579,2.958405447,-2.440736181,1.155965782,2.818210351,1.948078809,-0.071687144,-0.746723923,0.682928012,-0.392833048,-0.491834625,-0.490242192])
resOpt = sp.optimize.minimize(
        fun = call_lent,
        x0 = initial,
        args = (C, data_att, w_p,Nido+100, Nscenarioo,Nalter,Nattri,L,demo),
...
EST[0] = 1/(1+np.exp(-resOpt['x'][0]));EST[1] = 1/(1+np.exp(-resOpt['x'][1]));EST[5] = 1+np.exp(resOpt['x'][5]);EST[9] = 1+np.exp(resOpt['x'][9])
ERR=(np.sqrt(np.diag(resOpt.hess_inv.todense()))).copy() #Standard erro
ERR[[0,1,5,9]]=trans(resOpt['x'],resOpt.hess_inv.todense())
...
###########################################return results########################################
info = f"""
Estimation summary for Latent-class DFT
------------------------------------------------------------------------------------
...
class 2
intercept.EV             {EST[10]:10.2f}   {ERR[10]:10.2f}     {Z_VAL[10]:10.2f}     {P_VAL[10]:10.2f} {SIG[10]}
\u03B2_RC                     {EST[7]:10.2f}   {ERR[7]:10.2f}     {Z_VAL[7]:10.2f}     {P_VAL[7]:10.2f} {SIG[7]}
\u03B2_OC                     {EST[8]:10.2f}   {ERR[8]:10.2f}     {Z_VAL[8]:10.2f}     {P_VAL[8]:10.2f} {SIG[8]}
t                        {EST[9]:10.2f}   {ERR[9]:10.2f}     {Z_VAL[9]:10.2f}     {P_VAL[9]:10.2f} {SIG[9]}

latent class
Experience <5 years?     {EST[11]:10.2f}   {ERR[11]:10.2f}     {Z_VAL[11]:10.2f}     {P_VAL[11]:10.2f} {SIG[11]}
Daily distance <350km?   {EST[12]:10.2f}   {ERR[12]:10.2f}     {Z_VAL[12]:10.2f}     {P_VAL[12]:10.2f} {SIG[12]}
Switch to EV even when
the EVs on road below 50%?{EST[13]:10.2f}   {ERR[13]:10.2f}     {Z_VAL[13]:10.2f}     {P_VAL[13]:10.2f} {SIG[13]}
Work 5 days and below?    {EST[14]:10.2f}   {ERR[14]:10.2f}     {Z_VAL[14]:10.2f}     {P_VAL[14]:10.2f} {SIG[14]}
------------------------------------------------------------------------------------
